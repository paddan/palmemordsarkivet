...
_STOPWORDS = frozenset(
    "och på i av med för till den det de en ett om är var ur från "
    "the of in at a an to by".split()
)


def _entity_matches(entity: str, text_lower: str) -> bool:
    """Returnerar True om entiteten förekommer i texten.

    Exakt match först; sedan ordnivå-fallback med ordgränser för sammansatta
    eller flerordsnamn (t.ex. händelsenamn med bindestreck som inte upprepas
    ordagrant). Kräver minst 2 signifikanta delar för att aktivera fallback
    — undviker falskt positiva för enkla ord.
    """
    if entity.lower() in text_lower:
        return True
    words = [w for w in re.split(r"[\s\-]+", entity.lower())
             if len(w) > 3 and w not in _STOPWORDS]
    if len(words) < 2:
        return False
    # Kräver att ordet inte är inbäddat i ett längre sammansatt ord
    return any(
        re.search(r"(?<![a-zåäö])" + re.escape(w) + r"(?![a-zåäö])", text_lower)
        for w in words
    )


async def extract_entities(question: str, context: str) -> dict[str, list[str]]:
    """Extrahera entiteter relevanta för frågan ur chunk-kontexten med Claude Haiku."""
    prompt = (
        f"Fråga som ställdes: {question}\n\n"
        "Analysera dessa textutdrag och extrahera ENBART namngivna entiteter som är "
        "relevanta för frågan ovan. Utelämna allt som inte har direkt koppling till frågan.\n\n"
        "Svara ENBART med giltig JSON — inga förklaringar, inga kodblock:\n"
        "{\n"
        '  "persons": ["Fullständigt Namn"],\n'
        '  "places": ["Gatunamn / Byggnad / Stad / Lokal"],\n'
        '  "events": ["Namn på incident, spår, observation eller operation"]\n'
        "}\n\n"
        f"Textutdrag:\n{context[:4000]}"
    )
...
def build_graph_html(hits: list[dict], entities: dict[str, list[str]]) -> str:
    """Bygg interaktiv pyvis-graf av entitetssamförekomst i chunk-träffarna."""
    try:
    ...
    for h in hits:
        ...
        found = sorted(
            e for e in all_entities if _entity_matches(e, text_lower)
        )
        for e in found:
            node_count[e] += 1
    ...
    if submitted and q.strip():
        ...
        if _has_claude:
            with st.spinner("Extraherar entiteter för graf…"):
                try:
                    _ents = asyncio.run(extract_entities(q, format_context(hits)))
...
if ss.hits and not (mcp_mode and backend["kind"] == "claude"):
    ...
    if ss.graph_html:
        with st.expander("Entitetsgraf", expanded=False):
            ...
            st.components.v1.html(ss.graph_html, height=680, scrolling=False)
