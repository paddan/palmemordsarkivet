...
Claude Haiku) och lagrar parsad JSON i state.db-tabellen ``doc_entities``
(idempotent per pdf_stem+sida). Ladda till Neo4j med ``load_neo4j.py``
efteråt.

Kör:
    python -m graph.extract_entities [--limit 5] [--dry-run]
    python -m graph.extract_entities --provider openai --model gpt-4o-mini
"""
...
- Personnamn skrivs alltid som "Förnamn Efternamn" — aldrig "Efternamn, Förnamn"
  och inte enbart efternamn när förnamnet framgår av sidan. Använd samma form
  för en och samma person genom hela sidan så att den inte dubbleras.
...
def resolve_provider_cfg(saved: dict, *, provider: str = "", model: str = "",
    ...
    Samma semantik som llm_correct: CLI-provider ogiltigförklarar sparad modell
    (annars kan t.ex. --provider openai ärva en claude-modell)."""
...
async def extract_doc(conn, txt_path: Path, *, cfg: dict, dry_run: bool,
                      timeout: float = 120.0, jobs: int = 1,
                      on_page=None) -> int:
    """Extrahera alla kvarvarande sidor i ett dokument. Returnerar antal sidor.

    ``on_page(page_num)`` anropas efter varje försökt sida (lyckad eller med
    LLM-fel), men inte vid dry-run (då görs inga LLM-anrop alls).

    ``timeout`` är max väntetid (sekunder) per LLM-anrop — vissa anrop kan
    annars hänga i flera minuter. ``jobs`` styr hur många sidor som
    bearbetas samtidigt (via en semafor); sidorna i ett dokument är
    ...
    async def handle_page(page_num: int, text: str) -> None:
        async with semaphore:
            try:
                ...
                # sqlite3-anslutningen delas mellan korutiner, men eftersom
                # alla DB-skrivningar sker synkront i event loop-tråden
                # (korutiner interfolieras bara vid await-punkter, och
                # record_doc_entities gör inga await:s) är det säkert.
                state_db.record_doc_entities(
                    conn, pdf_stem=stem, page_num=page_num, payload=payload,
                    model=cfg["model"],
                )
            except TimeoutError:
                msg = f"timeout efter {timeout:.0f}s"
                print(f"  [{stem} p{page_num}] FEL: {msg}", file=sys.stderr)
                log_error("extract_entities", f"{txt_path.name}#p{page_num}", msg)
            except Exception as e:  # noqa: BLE001
                print(f"  [{stem} p{page_num}] FEL: {e}", file=sys.stderr)
                log_error("extract_entities", f"{txt_path.name}#p{page_num}", str(e))
            finally:
